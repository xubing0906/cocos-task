...
| :---| :--- |
| **Enabled** | 勾选该项即可开启天空盒 |
| **Env Lighting Type** | 环境光类型，详情请参考下文 **使用 IBL** 部分的内容。 |
...
| **DiffuseMap**      | Env Lighting Type下拉菜单中选择 **DIFFUSEMAP_WITH_REFLECTION** 属性后，自动生成的用于高级漫反射的卷积图，不支持手动编辑。 |
...
| **Env Lighting Type** |下拉菜单中选择**HEMISPHERE_DIFFUSE**项后，场景中的物体只能使用半球光方式的漫反射照明。
| **Env Lighting Type** |下拉菜单中选择**AUTOGEN_HEMISPHERE_DIFFUSE_WITH_REFLECTION**项后，则场景中的物体使用的是天空盒产生的环境照明的镜面反射。
| **Env Lighting Type** |下拉菜单中选择**DIFFUSEMAP_WITH_REFLECTION**项后，场景物体将使用更精确的漫反射图来取代默认的半球光照。详情请参考下文 **漫反射照明** 部分的内容。

### 漫反射照明

Creator 支持以下两种方式的环境漫反射照明，可通过选择**Env Lighting Type**下拉菜单中的**AUTOGEN_HEMISPHERE_DIFFUSE_WITH_REFLECTION**和 **DIFFUSEMAP_WITH_REFLECTION** 项来切换使用。

1. **半球光**：不选择 **DIFFUSEMAP_WITH_REFLECTION** 项时，使用半球光漫反射。该方式由 **Ambient** 组件中的 **SkyLightingColor** 和 **GroundLightingColor** 属性控制，渲染性能更高，但是细节度不够，照明方向性差。**可手动调节，但可能会和环境贴图变得不统一**。

    ![ambient-diffuse](skybox/ambient-diffuse.png)

2. **卷积图**：选择 **DIFFUSEMAP_WITH_REFLECTION** 项时，使用卷积图漫反射。该方式是高级漫反射，可以正确表达环境贴图产生的漫反射照明，有较好的照明方向性和细节。但 **DiffuseMap** 属性中的卷积图是自动生成来进行漫反射的，不允许手动编辑。
