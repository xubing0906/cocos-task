import {ReflectionProbeManager,_decorator, Component, Node, RenderTexture, director, Director, Camera, native, renderer, Label, Root, Sprite, SpriteFrame, resources, assetManager, ImageAsset, Texture2D, Light, Color, Quat, UITransform, Vec2, Size, Material, MeshRenderer } from 'cc';
...
export class renderTextureTest extends Component {
    ...
    private _height = 0;
    start() {
        const meshrender = this.node.getComponent(MeshRenderer);
        if (meshrender!==null) {
            console.log(meshrender.name);
