...
import { _decorator, Component, Button, labelAssembler, game ,director,Node, Scene, renderer,CameraComponent, Label, TextureCube} from 'cc';
...
export class SwitchHDR extends Component {
    ...
    {
        director.root!.pipeline.pipelineSceneData.isHDR = !this._skyBox.useHDR;
        this._skyBox.useHDR=!this._skyBox.useHDR;
        if (this._skyBox.envmap) {
            this._skyBox.useIBL=true;   
        }else{
            this._skyBox.useIBL=false;
        }
        this.setLabelContent();
    }
